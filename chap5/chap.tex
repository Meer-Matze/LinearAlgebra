...
\begin{theorem}
    ...
    \begin{enumerate}[label=\Roman*.]
        \item 对于任意特征值 $\lambda$，几何重数总是小于等于代数重数．
        \item 当且仅当所有特征值的几何重数等于代数重数时，矩阵 $A$ 可对角化．
    \end{enumerate}
...
\begin{proof}
    ...
    因$\bm{\alpha}_1 \neq \bm{0}$，故$k_3=0$. 回代得$k_1=k_2=k_3=0$，
    故向量组线性无关．
\end{proof}
...
由谱分解定理：
\begin{align*}
    \bm{A} &= \lambda_1 \frac{\bm{\alpha}_1\bm{\alpha}_1^T}{\|\bm{\alpha}_1\|^2} + \lambda_2 \frac{\bm{\alpha}_2\bm{\alpha}_2^T}{\|\bm{\alpha}_2\|^2} + \lambda_3 \frac{\bm{\alpha}_3\bm{\alpha}_3^T}{\|\bm{\alpha}_3\|^2} \\[1em]
    &= -1 \cdot \frac{1}{2} \begin{bNiceMatrix} 1 & 0 & -1 \\ 0 & 0 & 0 \\ -1 & 0 & 1 \end{bNiceMatrix} + 1 \cdot \frac{1}{2} \begin{bNiceMatrix}[columns-width = 1em] 1 & 0 & 1 \\ 0 & 0 & 0 \\ 1 & 0 & 1 \end{bNiceMatrix} + \bm{0} \\[1em]
    &= \begin{bNiceMatrix}[columns-width = 1.2em] 0 & 0 & 1 \\ 0 & 0 & 0 \\ 1 & 0 & 0 \end{bNiceMatrix}
\end{align*}
\end{solution}
\begin{example}
...
\begin{solution}
    ...
    \[ 
    \bm{A}\bm{P} = (\bm{A}\bm{\alpha}, \bm{A}^2\bm{\alpha}, \bm{A}^3\bm{\alpha})=(\bm{\alpha}, \bm{A}\bm{\alpha}, \bm{A}^2\bm{\alpha}) \begin{bNiceMatrix}[columns-width = 1em] 0 & 0 & 0 \\ 1 & 0 & 3 \\ 0 & 1 & -2 \end{bNiceMatrix} = \bm{P}\bm{C} 
    \]
    故 $\bm{A} \sim \bm{C} = \begin{bNiceMatrix}[columns-width = 1em] 0 & 0 & 0 \\ 1 & 0 & 3 \\ 0 & 1 & -2 \end{bNiceMatrix}$．
    $\bm{C}$ 的特征多项式 $|\lambda\bm{E}-\bm{C}| =\lambda(\lambda-1)(\lambda+3)$．
    特征值为 $0, 1, -3$．
    故 $|\bm{A}| = 0 \cdot 1 \cdot (-3) = 0$．\\
    $\bm{A}+3\bm{E}$ 的特征值为 $3, 4, 0\implies|\bm{A}+3\bm{E}|=0$\\
    $\bm{A}-\bm{E}$ 的特征值为 $-1, 0, -4\implies|\bm{A}-\bm{E}|=0$\\
    $\bm{A}+\bm{E}$ 的特征值为 $1, 2, -2\implies|\bm{A}+\bm{E}|=-4$
\end{solution}
\begin{exercise}
    设$\bm{A}=\begin{bNiceMatrix}
        ...
        2 & x & -2\\
        0 & 0 & -2
    \end{bNiceMatrix}\text{和}\bm{B}=\begin{bNiceMatrix}[columns-width = 1em]
        2 & 1 & 0\\
        0 & -1 & 0\\
        0 & 0 & y
...
\begin{enumerate}
    ...
    \[
    2\bm{E}-\bm{A} = \begin{bNiceMatrix} 4 & 2 & -1 \\ -2 & -1 & 2 \\ 0 & 0 & 0 \end{bNiceMatrix} \xrightarrow{\text{化简}} \begin{bNiceMatrix}[columns-width = 1em] 2 & 1 & 0 \\ 0 & 0 & 1 \\ 0 & 0 & 0 \end{bNiceMatrix}
    \]
    取对应特征向量 $\bm{\xi}_1=(1, -2, 0)^T$.
    \item 当 $\lambda_2=-1$ 时，解 $(-\bm{E}-\bm{A})\bm{x}=\bm{0}$.
    \[
    -\bm{E}-\bm{A} = \begin{bNiceMatrix} 1 & 2 & -1 \\ -2 & -4 & 2 \\ 0 & 0 & 1 \end{bNiceMatrix} \xrightarrow{\text{化简}} \begin{bNiceMatrix}[columns-width = 1em] 1 & 2 & 0 \\ 0 & 0 & 1 \\ 0 & 0 & 0 \end{bNiceMatrix}
    \]
    取对应特征向量 $\bm{\xi}_2=(-2, 1, 0)^T$.
    \item 当 $\lambda_3=-2$ 时，解 $(-2\bm{E}-\bm{A})\bm{x}=\bm{0}$.
    \[
    -2\bm{E}-\bm{A} = \begin{bNiceMatrix} 0 & 2 & -1 \\ -2 & -5 & 2 \\ 0 & 0 & 0 \end{bNiceMatrix} \xrightarrow{\text{化简}} \begin{bNiceMatrix}[columns-width = 1em] 4 & 0 & 1 \\ 0 & 2 & -1 \\ 0 & 0 & 0 \end{bNiceMatrix}
    \]
...
\begin{enumerate}
    ...
    \[ \begin{bNiceMatrix}[columns-width = 1em] 0 & -1 & 0 \\ 0 & 3 & 0 \\ 0 & 0 & 4 \end{bNiceMatrix} \bm{x}=\bm{0}\]
     取对应特征向量 $\bm{\eta}_1=(1, 0, 0)^T$.
    \item 当 $\lambda_2=-1$ 时，解$(-\bm{E}-\bm{B})\bm{x}=\bm{0} $\[ \begin{bNiceMatrix}[columns-width = 1em] -3 & -1 & 0 \\ 0 & 0 & 0 \\ 0 & 0 & 1 \end{bNiceMatrix} \bm{x}=\bm{0}\] 
    取对应特征向量 $\bm{\eta}_2=(1, -3, 0)^T$.
    \item  当 $\lambda_3=-2$ 时，解$(-2\bm{E}-\bm{B})\bm{x}=\bm{0}$ \[\begin{bNiceMatrix}[columns-width = 1em] -4 & -1 & 0 \\ 0 & -1 & 0 \\ 0 & 0 & 0 \end{bNiceMatrix} \bm{x}=\bm{0}\] 
    取对应特征向量 $\bm{\eta}_3=(0, 0, 1)^T$.
\end{enumerate}
从而 $\bm{P}_2 = \begin{bNiceMatrix}[columns-width = 1em] 1 & 1 & 0 \\ 0 & -3 & 0 \\ 0 & 0 & 1 \end{bNiceMatrix}$．\\[1em]
取$\bm{P} = \bm{P}_1 \bm{P}_2^{-1} = \begin{bNiceMatrix} 1 & 1 & -1 \\ -2 & -1 & 2 \\ 0 & 0 & 4 \end{bNiceMatrix}$．
\end{solution}
